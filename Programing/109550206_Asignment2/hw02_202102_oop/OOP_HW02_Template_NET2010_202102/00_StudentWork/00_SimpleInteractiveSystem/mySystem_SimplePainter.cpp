...
//Return x + y*nx
int SIMPLE_PAINTER::computeCanvasIndex( int x, int y, int nx, int ny ) const
{
    return x + y * nx;
}
	...
	for ( int j = -s; j <= s; ++j ) {
		for ( int i = -s; i <= s; ++i ) {
			...
			if ( R2 > S2 / 2.0 ) {    // pixel is a bit far but not too far
				...
				w = exp( d * k ); // use the guassian function to compute weight w
			} else {
...
{
	if ( Inside0toX(x, mNX) && Inside0toX(x, mNY) && Inside0toX(color.r, 1) && Inside0toX(color.g, 1) && Inside0toX(color.b, 1)) {
		int index = computeCanvasIndex(x, y, mNX, mNY);
...
bool SIMPLE_PAINTER::Inside0toX(double t, double x)
{
	return t >= 0 && t <= x;
}