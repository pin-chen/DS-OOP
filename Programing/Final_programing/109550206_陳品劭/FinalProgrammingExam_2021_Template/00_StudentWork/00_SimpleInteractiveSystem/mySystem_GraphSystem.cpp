    ...
    ) const
{
    ...
    for ( int i = 0; i < mCurNumOfActiveNodes; ++i ) {
        ...
        GRAPH_NODE *cn = &mNodeArr_Pool[ id ];
        double dx = cn->p.x - x;
        double dz = cn->p.z - z;
...
int GRAPH_SYSTEM::getNumOfAttachedEdgesOfSelectedNode( ) const
{
    ...
    //
	numAttachedEdges = mSelectedNode->edgeID.size();
    ...
    ) const
{
    ...
    //
	p = mNodeArr_Pool[nodeID].p;
	r = mNodeArr_Pool[nodeID].r;
}
...
int GRAPH_SYSTEM::getNumOfAdjacentNodesOfSelectedNode( ) const
{
    ...
    // of the selected node
    //
	numAdjacentNodes = mSelectedNode->edgeID.size();
