...
protected:
    ...
    //
	//for 1 2 3
	void generateNumbers(int range);
	//for move
	void moveOneLine(int i, int direction);
	int findItem( int start, int i, int direction ) const;
	void moveItem(  int now, int move, int i, int direction  );
	//for Previous
	void copyT( int src[][MAX_2048_NX], int tgt[][MAX_2048_NX]);
	//for auto
	double EventDFS( int numSteps, unsigned char action_key,int board[][MAX_2048_NX]);
	//
public:
    MY_2048( );
